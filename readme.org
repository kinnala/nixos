...
my game and apply declarative configuration to my operating system.
...
The above file defines my computing environment including the software packages.
It also pins down the versions of Emacs packages via a MELPA overlay.  I could
go further and fix also the version of nixpkgs (NixOS package repository) if
needed.
