#+TITLE: Declarative personal computing
...
system with its declarative description to determine the next action.
...
and a clear vision of its preferred state.  Hence, it is possible for me to
incorporate better tools and apply the principles of declarative configuration
...
The above file defines the software packages in my computing environment.  It
also pins down the versions of Emacs packages via a MELPA overlay.  I could go
further and fix also the version of nixpkgs (NixOS package repository) if needed.

Thus, transferring the computing environment to a new laptop is as simple as
installing NixOS, copying over the above file and running
