...
state of the system instead of individual steps for achieving it.  Such a
practice requires advanced tools that automagically compare the current state of
the system with its declarative configuration for determining the next action.
...
Today, I have strong opinions on how my computing environment should look
like and a clear vision of its preferred state.  Hence, I realized that
I should improve my tools accordingly.

* Enter NixOS

NixOS is a GNU/Linux system with an unique approach to configuration and
package management.  The preferred state of the operating system is defined
in a single file:
...
The configuration is rebuilt using
