#+TITLE: Declarative personal computing with NixOS
...
system with its declarative description for determining the next action.

In the context of cloud software deployment, one example of a declaratively
configured system is Kubernetes which has changed how applications are deployed
to the cloud.  The benefits of such an approach are clear: it is safer to deploy
updates when reverting is as simple as rolling back the configuration.

Today, I have strong opinions on how my computing environment should look like
and clear vision of its preferred state.  Hence, I realized that improving
my tools could be beneficial.

* Define state using configuration.nix

NixOS is a GNU/Linux system with an unique approach to configuration and package
management.  The preferred state of the operating system installation is defined
in a single file:
...
The above file defines the software packages I use in my computing environment.
It also pins down the versions of Emacs packages via a MELPA overlay.  I could
go further and fix also the version of nixpkgs (NixOS package repository) but
currently it is not done.

Thus, transferring the computing environment to a new computer should be as
simple as installing NixOS, copying this file over and running
...
Contents of this file get tangled to /etc/nixos via
...
(use-package jupyter
  :init
  (defun activate ()
    "Reload jupyter kernelspecs after activation."
    (interactive)
    (progn
      (venv-workon)
